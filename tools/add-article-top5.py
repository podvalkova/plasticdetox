...
ROWS = {
    ...
    "Thinkbaby": [("Thinksport mineral sunscreen SPF 50+", [["thinksport", "sunscreen"], ["thinksport", "spf"]], [], ["B00K3JQO9Y"], "careful",
                   "Same manufacturer as Thinkbaby, where independent third party testing by Lead Safe Mama in June 2025 found the highest lead level of any baby sunscreen tested to date. The mineral filter is the right chemistry, but the contamination result is the deciding fact.", F(te="caution"))],
    "Coppertone": [("Sport Mineral Sunscreen SPF 50", [["coppertone", "mineral"]], [], ["B08PCCWLZS"], "skip",
                    "Zinc oxide is the right filter, but the published inactive list names cyclopentasiloxane, a cyclic siloxane the EU restricts in leave on products, and polyester as a film former, so the formula fails under rule 2.1. This covers the Sport Mineral line only; the wider Coppertone range is chemical filter based.", F(fo="pass"))],
    # ------------------------------------------------------------ baby bottles
    ...
    # ------------------------------------------------------- laundry detergent
    "Tide": [("Tide liquid detergent and PODS", [["tide", "detergent"], ["tide", "pods"], ["tide", "liquid"]], [], ["B0BNWCTNYN", "B0BJMV9BXJ", "B085V5PPP8"], "unrated",
              "Heavy undisclosed fragrance load and a large single use plastic jug. PODS add a polyvinyl alcohol film that dissolves into the wash and the waterway rather than disappearing. Detergent is rinsed and diluted, so the exposure is lower than a leave on product.", F())],
    ...
    # ---------------------------------------------------------- shower curtains
    "Barossa Design": [("OEKO-TEX fabric shower curtain liner", [["barossa"]], [], ["B08QRFZ6TH"], "unrated",
                        "OEKO-TEX certified fabric, which is a real certification and a clear step up from a vinyl liner. Still a synthetic fabric, so it is the better plastic rather than no plastic.", F(te="pass"))],
    "LiBa": [("PEVA shower curtain liner", [["liba", "shower"], ["liba", "curtain"]], [], ["B00LS9UD2M"], "careful", PEVA_CURTAIN, F(fo="caution"))],
    # --------------------------------------------------------------- toothpaste
    "Crest": [("Crest whitening and 3D White pastes", [["crest", "toothpaste"], ["crest", "3d", "white"], ["crest", "scope"]], [], ["B005PLQIQ4", "B09F8FZ18G"], "careful",
               "Lead at about 399 ppb, with titanium dioxide and artificial dyes. Independent lab testing published by Lead Safe Mama in 2025. The brands dispute these results.", F(te="caution", fo="caution"))],
    "Colgate": [("Colgate Optic White and Cavity Protection", [["colgate", "optic", "white"], ["colgate", "cavity"], ["colgate", "toothpaste"]], [], ["B082F1QH7S", "B01BNEWDFQ"], "careful",
                 "Lead at about 539 ppb, the highest of the mainstream pastes, alongside titanium dioxide and SLS. Independent lab testing published by Lead Safe Mama in 2025. Colgate disputes these results.", F(te="caution", fo="caution"))],
    ...
    # ------------------------------------------------------------ water bottles
    "Owala": [("FreeSip bottles", [["owala"]], [], ["B0BZYCJK89"], "skip",
    ...
    "CAP": [("CAP thick exercise mats", [["cap", "yoga", "mat"]], [], ["B0C7SFV8RH"], "unrated", MAT, F())],
    ...
    # ------------------------------------------------------------ protein powder
    "Optimum Nutrition": [("Gold Standard whey and creatine", [["optimum", "nutrition"]], [], ["B000GISTZ4", "B000QSTBNS", "B002DYIZHG", "B000QSNYGI", "B002DYIZEO", "B002DYIZEE"], "unrated",
                           "Informed Choice certified on most lots, which covers banned substances rather than heavy metals or plastic. Protein powder is a category where independent testing has repeatedly found lead and cadmium, and there are no published metal numbers here to check.", F())],
    "Body Fortress": [("Super Advanced whey protein", [["body", "fortress"]], [], ["B0BJLBD427"], "careful",
                       "No third party certification for heavy metals or banned substances, with artificial sweeteners and colours. Protein powders are one of the categories where independent testing has repeatedly found lead and cadmium, so an uncertified daily powder is an unknown.", F(fo="caution"))],
    # ------------------------------------------------------------- dental floss
    ...
    # -------------------------------------------------------------- body lotion
    "eos": [("eos Shea Better body lotion", [["eos", "lotion"], ["eos", "shea"]], [], ["B08KT2Z93D", "B0BZGRCBY4"], "skip",
    ...
    "CeraVe": [("Invisible Mineral Sunscreen SPF 50", [["cerave", "mineral", "sunscreen"]], [], ["B0FXNHDWM7"], "unrated",
               ...
               ("Daily Moisturizing Lotion", [["cerave", "moisturizing", "lotion"], ["cerave", "daily"]], ["sunscreen", "mineral"], ["B07RK4HST7"], "unrated",
    ...
    "Jergens": [("Jergens body lotion", [["jergens"]], [], ["B0FKC23886"], "skip",
    ...
    "Method": [("Method all purpose cleaner sprays", [["method", "all", "purpose"], ["method", "cleaner"]], [], ["B007AHO6CO", "B0BGQTZB4T"], "unrated",
                "Better ingredient disclosure than the disinfectant brands, but fragranced and sold in a single use plastic spray bottle. A refill tablet system removes the bottle entirely.", F())],
    # ------------------------------------------------------------ crib mattress
    ...
    # ---------------------------------------------------------------------- tea
    "Lipton": [("Lipton tea bags", [["lipton", "tea"]], [], ["B0DX5XQNRH"], "careful", TEA, F(fo="caution", te="caution"))],
    "Twinings": [("Twinings tea bags", [["twinings"]], [], ["B001GM60LE", "B0DYK97R27"], "skip", TEA, F(fo="fail", te="fail"))],
    "Bigelow": [("Bigelow tea bags", [["bigelow"]], [], ["B001A3OADO"], "careful", TEA, F(fo="caution", te="caution"))],
    ...
    "POWCAN": [("POWCAN insulated bottle with straw lid", [["powcan"]], [], ["B0D8J2ZB8P"], "skip", STRAW_LID, F(pk="fail"))],
    "Triple Tree": [("Triple Tree insulated bottle", [["triple", "tree", "bottle"], ["triple", "tree", "insulated"]], [], ["B07Z4L57XQ"], "skip", STRAW_LID, F(pk="caution"))],
    "DYSANKY": [("DYSANKY insulated bottle with straw lid", [["dysanky"]], [], ["B0DLCG65B5"], "skip", STRAW_LID, F(pk="fail"))],
    "Fijinhom": [("Fijinhom insulated bottle", [["fijinhom"]], [], ["B0D2W136D1"], "skip", STRAW_LID, F(pk="caution"))],
    ...
    "BulkSupplements": [("BulkSupplements creatine monohydrate", [["bulksupplements"]], [], ["B00E9M4XEE"], "unrated",
    ...
    "AmazonBasics": [("Amazon Basics exercise and yoga mats", [["amazon", "basics", "yoga"], ["amazon", "basics", "exercise"]], [], ["B01LP0U5X0", "B01LP0UX9G"], "careful",
                      MAT, F(fo="caution"))],
    # ------------------------------------------------------------- bottled water
